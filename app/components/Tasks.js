...
export default function Tasks({ tasks, user, onAdd, onComplete }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-5 py-5 border-b border-black/8 flex items-baseline justify-between">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {tasks.map((task) => (
          <div key={task.id} className="flex items-start gap-4 px-5 py-5 border-b border-black/8">
            <button
            ...
            <div className="flex-1">
              <p className="text-[15px] font-light text-t1" style={{ lineHeight: "1.5" }}>{task.text}</p>
              {task.owner && (
